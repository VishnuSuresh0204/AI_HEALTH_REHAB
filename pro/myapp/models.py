...
class ExerciseSession(models.Model):
    ...
    recorded_video = models.FileField(
        ...
        null=True,
        blank=True,
        help_text="Original video the patient uploaded of themselves performing the exercise"
    )

    annotated_video = models.FileField(
        upload_to="session_recordings/analyzed/",
        null=True,
        blank=True,
        help_text="AI-processed copy with the pose skeleton drawn over each frame"
    )
    ...
    rep_phase = models.CharField(
        max_length=20,
        default="extended"  # extended / flexed — used by the AI rep counter state machine
    )
...
class ActivityLog(models.Model):
    ...
    def __str__(self):
        return f"{self.loginid} - {self.action}"